...
class Graph:
    ...
    def minimax_eval(self, node,
                     maximizingPlayer):  # rekursīva funkcija, kas pēc minimaksa algoritma katrai virsotnei piešķir heiristisko vertējumu
        ...
        if maximizingPlayer:  # ja spēlētājs, kuram jāizdara gājiens, ir maksimizējošais spēlētājs(virsotne atrodas Max līmenī)
            ...
            for child_node in node.ChildNodes:
                child_node.eval = self.minimax_eval(child_node,
                                                    False)  # ja virsone nav strupceļa virsotne, uz to bērniem tiek pielietots minimaks, pieņēmot, ka tie atrodas Min līmenī
            ...
            for child_node in node.ChildNodes:
                child_node.eval = self.minimax_eval(child_node,
                                                    True)  # ja virsone nav strupceļa virsotne, uz to bērniem tiek pielietots minimaks, pieņēmot, ka tie atrodas Max līmenī
                min_eval = min(min_eval, child_node.eval)
            node.eval = min_eval
            return min_eval

    def heuristic(
            self):  # visiem strupceļa virsotnēm piešķir heiristisko vērtējumu, kas ir vienāda ar 2. un 1. spēlētāja punktu starpību
        for key, value in self.nodes.items():
            if not value.ChildNodes:
                value.eval = value.p2_score - value.p1_score

    def alfa_beta_eval(self, node, alpha, beta,
                       maximizingPlayer):  # rekursīva funkcija, kas pēc alfa-beta algoritma katrai virsotnei piešķir heiristisko vertējumu
        self.visited_nodes_count += 1  # izseko cik virsotnes bija apmeklētas
        ...
        if maximizingPlayer:  # ja spēlētājs, kuram jāizdara gājiens, ir maksimizējošais spēlētājs(virsotne atrodas Max līmenī)
            ...
            # funkcija izvēlas maksimālo vērtību no pieejamajiem bērnu virsotnēm un atgriež šo vērtību
            for child_node in node.ChildNodes:
                child_eval = self.alfa_beta_eval(child_node, alpha, beta,
                                                 False)  # ja virsone nav strupceļa virsotne, uz to bērniem tiek pielietota alfa-beta, pieņēmot, ka tie atrodas Min līmenī
                max_eval = max(max_eval, child_eval)
                alpha = max(alpha,
                            max_eval)  # labākā (lielākā) vērtība, ko maksimizējošais spēlētājs varētu sasniegt šajā līmenī
            ...
            for child_node in node.ChildNodes:
                child_eval = self.alfa_beta_eval(child_node, alpha, beta,
                                                 True)  # ja virsone nav strupceļa virsotne, uz to bērniem tiek pielietota alfa-beta, pieņēmot, ka tie atrodas Max līmenī
                min_eval = min(min_eval, child_eval)
                beta = min(beta,
                           min_eval)  # labākā (mazākā) vērtība, ko minimizējošais spēlētājs varētu sasniegt šajā līmenī
...
def minimax(startNum, p1_score, p2_score):  # funkciija, kas izpilda alfa-beta algoritmu un izpilda gājienu
    ...
    graph.minimax_eval(graph.nodes[0],
                       True)  # funkcija pēc minimaksa algoritma katrai virsotnei piešķir heiristiskus vertējumus
...
def alphabeta(startNum, p1_score, p2_score):  # funkciija, kas izpilda alfa-beta algoritmu un izpilda gājienu
    ...
    graph.generate_graph(startNum, p1_score, p2_score)  # funkcija ģenerē grāfa daļu no uzdotas virsotnes
    graph.heuristic()  # funkcija piešķir strupceļa virsotnem heiristiskus vertējumus
    graph.alfa_beta_eval(graph.nodes[0], float('-inf'), float('inf'),
                         True)  # funkcija pēc alfa-beta algoritma katrai virsotnei piešķir heiristiskus vertējumus
    best_child = graph.choose_best_child()  # funkcija izvēlas nākamo gajiēnu kā virsotni ar lielāko heiristisko vērtējumu
    best_child.visited_nodes = graph.visited_nodes_count  # funkcija piešķir šai virsotnei apmēklēto virsotņu skaitu, lai pēc tām to varētu izvadīt
    return best_child
